function leader_modeling7

% This model charge will be deposite at the optimum previous point whether
% it a PBFA or HSVP point. 
% Also it will try to match the early part of the leader too.
% Added/deleted from previous version
%   From version 5 to 6
%       - The blind leader in version 5 modeled with putting point on a
%       straight line. But the real point(s) could be any where in that
%       plane. This program will try to find a optimum horizontal position
%       for a given plane.
...
% Base folder
bf = 'C:\Users\daqop\Desktop\sumedhe\HSV analasis\20110814\F01\1stRS\';
...
% Last PBFA point
lastPbfa = [77358.4569971	-12798.2	1466.8	4442.7];

% For horizontal grid search
xgrid = -14000:200:-11500;
ygrid = 0:200:3000;

% save figures?
save_figure = 1;
% Comments to include at the end of the file name
comment    = 'Algorythm7-200m-20us';        
...
N0 = round((b.firstHSVP(1) - b.lastPbfa(1))/20e-6);
...
% Points to deposite positive charges
positives.t = [b.pbfa(:,1);b.t];
positives.x = [b.pbfa(:,2);b.x];
positives.y = [b.pbfa(:,3);b.y];
positives.z = [b.pbfa(:,4);b.z];
positives.q = positives.t - positives.t;
...
for i = 2:L-1
    ...
    llll = find(b.t < tuni(i));
    L1new = L1+llll(end);
    
    if i < L1 + N0       
        
        ki_vals = nan(Lx,Ly,L1new);
        dQ_vals = ki_vals;
        dE_vals = zeros(Lx,Ly,L1new,L0);
        ...
        for Lxi = 1:Lx
            ...
            for Lyi = 1:Ly
                b.y(ind) = ygrid(Lyi);
                for j = 1:L1new
                    b.xp = positives.x(j); b.yp = positives.y(j); b.zp = positives.z(j);
                    [dQ_vals(Lxi,Lyi,j) dE_vals(Lxi,Lyi,j,:) N ki_vals(Lxi,Lyi,j)] = ...
                        find_dQ4(dEms,b,ind,endInd(i),sen_set,sn,-.1,.1,0);
                    multiWaitbar( 'Charges for each PBFA', ((Lxi-1)*Ly*L1new +(Lyi-1)*L1new+j)/(L1new*Lx*Ly));
                end
            end
        end
        ...
        % get min indexes
        [mi mj mk] =  ind2sub(size(ki_vals), idx);
        
        
        
        b.x(ind) = xgrid(mi);
        b.y(ind) = ygrid(mj);
        positives.x(L1+ind) = xgrid(mi);
        positives.y(L1+ind) = ygrid(mj);
        
        
        dQ   = dQ_vals(mi,mj,mk);
        positives.q(mk) = positives.q(mk) + dQ;
        dEcs = dE_vals(mi,mj,mk,:);
        
    else
        ki_vals = nan(1,L1new);
        dQ_vals = ki_vals;
        dE_vals = zeros(L1new,L0);
        
        for j = 1:L1new
            b.xp = positives.x(j); b.yp = positives.y(j); b.zp = positives.z(j);
            [dQ_vals(j) dE_vals(j,:) N ki_vals(j)] = ...
                find_dQ4(dEms,b,ind,endInd(i),sen_set,sn,-.1,.1,0);
            multiWaitbar( 'Charges for each PBFA', j/L1new);
        end
        ...
        dQ   = dQ_vals(mind);
        positives.q(mind) = positives.q(mind) + dQ;
        dEcs = dE_vals(mind,:);
...
if q_distribution
    ...
    [N_x N_y] = ldar2scr(b,b.x,b.y,b.z);
    
    scatter(N_x,240-N_y,12,b.qs,'filled')
    ...
    % Plotting positive charge data
    indx = find(positives.q == 0);
    positives.q(indx) = NaN;
    
    [N_x N_y] = ldar2scr(b,positives.x,positives.y,positives.z); 
    
   
    scatter(N_x,240-N_y,abs(positives.q/min(positives.q)*400),[1 0 0])
