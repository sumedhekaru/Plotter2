function [t,E_stat,E_ind,E_rad,P] = IBP_mod_MTLK(maxA,t1,t2,v,H1,Hm,H2,x0,y0,T1,...
       T2,t_step,lamda1,lamda2,x,y,dh,alpha)
% User Inputs
if nargin == 0
    a=struct( ...
        'maxA' , {-420}        , ...
...
%         else
%             ii(j)  = exp((a.Hm-a.H1)/a.lamda1)*a.maxA*exp(-(z(j)-a.H1)/a.lamda2)*exp(-a.alpha^2*(t(j)-a.t1)^2);
%             didt(j)= exp((a.Hm-a.H1)/a.lamda1)*exp(-(z(j)-a.H1)/a.lamda2)*(-2)*a.maxA*a.alpha^2*(t(j)-a.t1)*exp(-a.alpha^2*(t(j)-a.t1)^2);
%         end
        
        % MTLR        
        % ii(j)  = a.maxA*(z(j)-a.H1)/sig*exp(-(z(j)-a.H1)^2/sig)*exp(-a.alpha^2*(t(j)-a.t1)^2);
        % didt(j)= (z(j)-a.H1)/sig*exp(-(z(j)-a.H1)^2/sig)*(-2)*a.maxA*a.alpha^2*(t(j)-a.t1)*exp(-a.alpha^2*(t(j)-a.t1)^2);
        
         % MTLKumaraswami
        ii(j)=a.maxA*(abs(z(j)-a.H1)/abs(a.H2-a.H1)).^(a.lamda1-1).*((1-(abs(z(j)-a.H1)/abs(a.H2-a.H1)).^a.lamda1).^(a.lamda2-1))*exp(-a.alpha^2*(t(j)-a.t1)^2);
        didt(j)=(abs(z(j)-a.H1)/abs(a.H2-a.H1)).^(a.lamda1-1).*((1-(abs(z(j)-a.H1)/abs(a.H2-a.H1)).^a.lamda1).^(a.lamda2-1))*(-2)*a.maxA*a.alpha^2*(t(j)-a.t1)*exp(-a.alpha^2*(t(j)-a.t1)^2);
...
%         else
%             ii(j)=exp((a.Hm-a.H1)/a.lamda1)*a.maxA*exp(-(z(j)-a.H1)/a.lamda2)*exp(-(a.alpha/a.k)^2*(t(j)-a.t1)^2);
%             didt(j)=exp((a.Hm-a.H1)/a.lamda1)*exp(-(z(j)-a.H1)/a.lamda2)*(-2)*a.maxA*(a.alpha/a.k)^2*(t(j)-a.t1).*exp(-(a.alpha/a.k)^2*(t(j)-a.t1)^2);
%             
%         end

        % MTLR
        % ii(j)=a.maxA*(z(j)-a.H1)/sig*exp(-(z(j)-a.H1)^2/sig)*exp(-(a.alpha/a.k)^2*(t(j)-a.t1)^2);
        % didt(j)=(z(j)-a.H1)/sig*exp(-(z(j)-a.H1)^2/sig)*(-2)*a.maxA*(a.alpha/a.k)^2*(t(j)-a.t1).*exp(-(a.alpha/a.k)^2*(t(j)-a.t1)^2);
        
        %MTLKumaraswami
         ii(j)=a.maxA*(abs(z(j)-a.H1)/abs(a.H2-a.H1)).^(a.lamda1-1).*((1-(abs(z(j)-a.H1)/abs(a.H2-a.H1)).^a.lamda1).^(a.lamda2-1))*exp(-(a.alpha/a.k)^2*(t(j)-a.t1)^2);
         didt(j)=(abs(z(j)-a.H1)/abs(a.H2-a.H1)).^(a.lamda1-1).*((1-(abs(z(j)-a.H1)/abs(a.H2-a.H1)).^a.lamda1).^(a.lamda2-1))*(-2)*a.maxA*(a.alpha/a.k)^2*(t(j)-a.t1).*exp(-(a.alpha/a.k)^2*(t(j)-a.t1)^2);
