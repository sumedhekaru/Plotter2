...
% please report any bugs or suggestions to oldenhuis@gmail.com   
    ...
    assert( (nargin == 1) || (mod(nargin,2) == 0),...
        'settingsdlg:incorrect_parametervalue_pairs',...
        'settingsdlg() requires an even number of arguments.');     
    ...
    total_width = 250;    width_ind = strcmpi(fields, 'WindowWidth');       
    if any(width_ind)
    ...
    % calculate best height for all uicontrols()
    ...
    if num_separators > 0
        text_offset_X = 20;
        text_width = (total_width-control_width-18);        
    else
        ...
        % or a setting? 
            ...
            % cells  : use popup
                ...
                controls(ii) = uicontrol(...
                    ...
                    'parent'  , fighandle,...
                    'string'  , values{ii},...
                    'position', [text_offset_X+text_width,control_offset_Y,...
    ...
    uicontrol(...
        ...
        'parent'  , fighandle,...
        'string'  , 'OK',...
        'position', [total_width*(1-1/2.5)-separator_offset_X,2, total_width/2.5,control_height*1.5],...
        'Callback', @OK)  
    ...
    function EnableDisable(which, varargin)
        ...
        if num_separators > 0
            index = find(separators == which);
            if index < length(separators)
                 range = (separators(index)+1):(separators(index+1)-1);
            else range = (separators(index)+1):numel(controls);
            end
