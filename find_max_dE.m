function data = find_max_dE(handles,opt)
...
for i=1:60
    ...
    if strcmp(a.lp_fn{i},'')==0
        ...
        dVn = vn(2:end)-vn(1:end-1);
        dEt = tn(2) -tn(1);
        %plot(tn,vn*gain(i)*factor(i)+vshift(i))
        if  opt.graphOut == true;
            plot(tn(1:end-1)+dEt/2,dVn*gain(i)*factor(i)/dEt)
        end
        maxdE=max(dVn)*gain(i)*factor(i);
        
        maxdE_value = [maxdE_value maxdE];
        maxdE_name  = [maxdE_name lp_legend{i}];
...
for i=1:60
    ...
    if strcmp(a.ch_fn{i},'')==0
        ...
        if isempty(t)==0
            ...
            if  opt.graphOut == true;
                plot(t(1:end-1)+dt/2,dy*gain(i)*factor(i))
            end
            
            maxdE=min(dy)*gain(i)*factor(i);
            
            maxdE_value = [maxdE_value maxdE];
            maxdE_name  = [maxdE_name ch_legend{i}];
...
if (~isnan(DLS(1,10)) || ~isnan(CG(1,10)) || ~isnan(LINET(1,3)) ...
        || ~isnan(PBFA(1,6)) || ~isnan(LINET2(1,3))) && opt.graphOut == true;
    try
...
if empty_plot==false
    ...
    if length(CG(:,10)) == 1 && length(cglss(:,1)) == 1
        
        % Distance to each sensor from CGLSS poing
        r = ((settings.x-CG(1,6)).^2 ...
                 +(settings.y-CG(1,7)).^2+(settings.z - CG(1,8).^2)).^0.5;
        % Print information
        if opt.textOut == true;
            fprintf('\nTime\t\t\tSensor\tMax dE\tDist\tPeak Curr\n')
        end
        
        for i=1:length(maxdE_name)
            maxdEn = maxdE_name{i};
            k = find(strcmp(settings.sen_IDs,maxdEn(1:3)),1);
            if opt.textOut == true;
                fprintf('%0.6f\t%s\t%0.2f\t%0.1f\t%0.1f\n',...
                    cglss(1,1),maxdE_name{i}, maxdE_value(i),r(k),cglss(1,11))
            end
            data.R = [data.R r(k)];
        end
    ...
    if opt.graphOut == true
        ...
        box on
        grid on
        title_str=sprintf('%s-%s-%s    %s   UT: %2.2i:%2.2i:%2.2i    %s\n ', ...
            g.YYYY{:},g.MM{:},g.DD{:},ld_lin,g.hh,g.mm,g.ss,ch_freq_str);
