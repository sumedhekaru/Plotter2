function IBP_to_RS_ratio_post_processing2
clc
%% key preses
% q -- quit the program
% s -- skip the file
...
for i = 1:L
    tempStr = fileNames{i};
    fileNumbers{i} = tempStr(1:5);
end

fileNumbers = unique(fileNumbers');
L = length(fileNumbers);


for i = 1:L

    key = start_working(bf,fileNumbers{i},sFn);
...
%% Funtions
function key = start_working(bf,fnum,sFn)

    %fprintf('\n********************************************************\n')
    fprintf('\n\t\t\tworking on the flash %s\n',fnum)
    fprintf('********************************************************\n')
    
    % Open the first figure available
    for i = 1:10
        try
            sprintf('%s%s-%2.2i.fig',bf,fnum,i)
            openfig(sprintf('%s%s-%2.2i.fig',bf,fnum,i),'new','invisible')  
            break
        end
    end
