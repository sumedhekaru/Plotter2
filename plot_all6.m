...
%                 from NLDN button
%       
%       % 2013-07-06 added NLDN plotting capabilities as NLDN2
%       % 2013-11-25 added Feild mill data plotting
%       % 2015-06-15 added additional plots for dEdt data handling,
%       perticularly, detrend, high-pass filter, and integration.
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function output = plot_all6(g,verbose)
%clc
if nargin < 2
...
if sum(strcmp(a.lp_fn,''))==60 && ...
        ...
        strcmp(a.nldn2_fn,'')== 1    && sum(strcmp(a.fm_fn,''))== 34
    if verbose
...
for i=1:60
    ...
    if strcmp(a.ch_fn{i},'')==0
        ...
        if isempty(t)==0
            ...
            % Is detrending on?
            if settings.deTrend(i)
                y = detrend(y);                
                % Let user know data were detrended
                ch_legend_str = [ch_legend_str '-d'];
            end
            
            
            % Do we need to do the notch filter?
            if settings.notch_center(i) > 0
                y = notch_filter_v01(t,y,settings.notch_center(i), ...
                    settings.notch_bw(i),settings.notch_at(i));
                
                ch_legend_str = [ch_legend_str sprintf('-nf(%0.0f,%0.0f,%0.0f)',settings.notch_center(i), ...
                    settings.notch_bw(i),settings.notch_at(i))];                
            ...
            % Do we need to (high pass) filter the data
            if settings.hp_filt(i) > 0
                
                %[t,y,filtered] = ch_high_pass(t,y,g.temp.ch_hpf);
                [t,y,filtered] = dedt_filt_Integrator(t,y,settings.hp_filt(i));
                
                ch_legend_str = [ch_legend_str sprintf('-%0.1fKHz',settings.hp_filt(i)/1000)];
            ...
            % Is user need to intigrate?
            if settings.intigrate(i)
                yInt = cumsim(t,y);
                yInt = yInt * settings.integration_gain(i);
                ch_legend_str2 = [ch_legend_str '-I\times' num2str(settings.integration_gain(i)) ];                    
            else
                yInt = [];
            end
                        
            % Does user need original data plot?
            if settings.plot_type(i) == 1 || settings.plot_type(i) == 3
                plot(t,y*gain(i)*factor(i)+vshift(i))
                
                if gain(i) ~= 1
                    ch_legend_str = [ch_legend_str '\times' num2str(gain(i))];
                end
                
                lg = [lg ch_legend_str];
            end
            
            % Does user need intigrated data plot
            if (settings.plot_type(i) == 2 && ~isempty(yInt)) || ...
                    (settings.plot_type(i) == 3 && ~isempty(yInt))
                plot(t,yInt*gain(i)*factor(i)+vshift(i))
                lg = [lg ch_legend_str2];            
            end
            
            
            
            output.ch_data(i).y = y*gain(i)*factor(i)+vshift(i);
            output.ch_data(i).t = t;
...
if ~isnan(DLS(1,10)) || ~isnan(CG(1,10)) || ~isnan(LINET(1,3)) ...
        ...
        || ~isnan(NLDN2c(1,1)) || ~isnan(NLDN2g(1,1))
    try
    ...
    % Added to fix R2014b onward lengend problems
    ...
    if sum(~isnan(NLDN2c(:,1)))>0 && g.nldn2
        plot (AX(2),NLDN2c(:,8),NLDN2c(:,4),'rs','MarkerFaceColor','r','MarkerSize',mz+2)
        lg = [lg 'NLDN2-c'  ];
        sec_y_on = true;
        empty_plot=false;
    end
    ...
    if sum(~isnan(NLDN2g(:,1)))>0 && g.nldn2
        plot (AX(2),NLDN2g(:,8),NLDN2g(:,4),'bs','MarkerFaceColor','b','MarkerSize',mz+2)
        lg = [lg 'NLDN2-g'  ];
        sec_y_on = true;
        empty_plot=false;
    end
