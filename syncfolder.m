...
p1 = 'C:\Users\sumedhe\Desktop\Plotter2';
p2 = 'E:\Sumedhe\MyDataAnalysis\Plotter2 Backup';
syncdirect = '1';
            ...
            if files1(nf1).datenum > files2(nf2).datenum + 1.0/24/60
                if syncdirect >= 0
                    display(['''' p1, files1(nf1).name ''' --> ''' ...
                        p2, files2(nf2).name '''']); 
                    backup_file_sumedhe(p2, files2(nf2))
...
function backup_file_sumedhe(p2, files2)

% backup the file name with date and time stam so that I can recover
[pathstr, name, ext] = fileparts([p2 files2.name]);

d= datevec(files2.date);
sec = d(4)*3600+d(5)*60+d(6); 
new_fn = sprintf('%s/%s_%4.4i%2.2i%2.2i_%5.5i%s',...
    pathstr,name,d(1),d(2),d(3),sec,ext);

copyfile([p2, files2.name],new_fn, 'f');
