...
% Last Modified by GUIDE v2.5 30-Mar-2015 19:18:34
...
function [xn,yn] = ax2norm(hAx,x,y)
...
function [xn,yn] = norm2ax(hAx,x,y)
...
handles.a = a;
handles = update_figD(handles,'new');
...
function handles = update_figD(handles,method)

a = handles.a;

% Get currrent figure data
figD = guidata(a.fig);

if strcmp(method,'new')
    if isempty(figD) || isempty(figD.objects)
        % add to figure handles, this is the first item
        figD.objects = a;
    else
        figD.objects = [figD.objects a];
    end
elseif strcmp(method,'update')
    if isempty(figD.objects)
        figD.objects = a;
    else
        figD.objects(end) = a;
    end
...
if strcmp(a.objectType,'line')
    ...
    xL = xlim(a.ax);
    Vq = interp1([a.x1 a.x2],[a.y1,a.y2],xL,'linear','extrap');
