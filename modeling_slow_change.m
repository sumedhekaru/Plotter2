...
for i = 2:Nh
    ...
    for j = 1:Ns
        ...
        E1 = interp1(data(j).t,data(j).v,ts(i-1),'pchip');
        dEs(j) = E2 - E1;
    ...
    %a.dQ = dQ;
    %a.lb = [2000 2000 2000 100];
    %a.ub = [2000 2000 2000 100];
    %[P2 ki dEc] = optimize_p2_2(a);
    %dQ = P2(4);
    
    for j = 1:Ns
        data(j).t2(i)  = ts(i);
        data(j).v2(i)  = data(j).v2(i-1) + dEc(j);
        data(j).kis(i) = ki;
        data(j).dQ(i)  = dQ;
        data(j).Q(i)   = data(j).Q(i-1) + dQ;
    end   
