...
bf = 'C:\Users\Sumedhe\Desktop\NBP_project_2013\NBP\20110814\Times\';
...
if ~exist(dsfn,'file')
    ...
    fprintf(fID,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t\n\n',...
        'Index     ','RiseTime  ','10-90RiseT','ZeroCrossT','NegPeakT  ','TotalTime ','FWHM      ');
else
    fID = fopen(dsfn,'a+');
end
...
% Horizontal distance
r = sqrt((sen_set.x(sns)-xyz(1)).^2+(sen_set.y(sns)-xyz(2)).^2);
...
% collect some info
if isfield(data,'ch_data') && range(data.ch_data(sns(I(sn_ind))*3).y) > 0.1
    
    % Original data
    yo = data.ch_data(sns(I(sn_ind))*3).y;
    to = data.ch_data(sns(I(sn_ind))*3).t;
