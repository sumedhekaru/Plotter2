...
bf = 'C:\Users\daqop\Desktop\Sumedhe\HSV analasis\20110814\F02\20140509-K02\';
...
save_output     = 0;    % Save model data in to a matlab structure
...
for i = 2:L-1
    ...
    for j = 1:L1
        ...
        [dQ_vals(j) dE_vals(j,:) N ki_vals(j) ki_vals2(j)] = ...
...
if ki_sqrd_plot
    figure
    plot(tuni(1:L-7),ki_squires2(1:L-7));
    xlabel('Time (s)')
    ylabel('\chi^2 (V^2/m^2)')
    title(sprintf('Max = %0.3f   Min = %0.3f  Avg = %0.8f ',...
        max(ki_squires2(1:L-7)),min(ki_squires2(1:L-7)),nanmean2(ki_squires(1:L-7))))
...
function  [dQ dEcs N ki ki2] = find_dQ4(dEms,b,ind,endInd,sen_set,sn,q1,q2,N)
%%    
    ...
    % Measurement errors
    mErr = [0.32 0.31 0.31 NaN 0.18 0.29 0.53 0.16 0.13 0.17 NaN]/1000;
    ...
    for i = 1:4
        ...
        % kis(i) = sum(((dEcs - dEms)./dEms).^2);
        kis(i) = sum(((dEcs(1) - dEms(1))./mErr(1)).^2)/2;
        
        kis2(i) = sum(((dEcs - dEms)./mErr(sn)).^2)/2;
        % kis(i) = sum((dEcs - dEms).^2);
    ...
    [dQ dEcs N ki ki2] = find_dQ4(dEms,b,ind,endInd,sen_set,sn,qs(indx)-dq,qs(indx)+dq,N);     
