function [t,E_stat,E_ind,E_rad,P,Is,didts] = IBP_mod_MTLEL(maxA,t1,t2,tt2,t3,v,H1,H2,x0,y0,T1,...
...
for j=1:length(t)
   ...
   elseif t(j) <= a.t1 + a.tt2             
        ...
        ii(j)=a.maxA*exp(-(z(j)-a.H1)/a.lamda)*exp(-(a.alpha/a.k)^2*(t(j)-a.t1)^2);
        didt(j)=exp(-(z(j)-a.H1)/a.lamda)*(-2)*a.maxA*(a.alpha/a.k)^2*(t(j)-a.t1).*exp(-(a.alpha/a.k)^2*(t(j)-a.t1)^2);
    elseif t(j) <= (a.t1 + a.tt2 + a.t3)        
        T = a.t1 + a.tt2;
        V0 = a.maxA*exp(-(z(j)-a.H1)/a.lamda)*exp(-(a.alpha/a.k)^2*(T-a.t1)^2);
        ii(j) = -V0*(t(j)-T-a.t3)/a.t3;
        didt(j) = -V0/a.t3;
    else
        ii(j) = 0;
        didt(j) = 0;
